...
<ul class="dropdown-menu">
	<li><a href="<%=path%>/DicType/add?"><i class="icon-pencil"></i>新增</a></li>
...
<form action="<%=path%>/DicType/list?" method="post" id="listForm"
	name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
			<th><input type="checkbox" name="all" onclick="selectAll('ids')"/></th>
			<th>${msg['label.DicType.VALUE_TYPE']}</th>
			<th>${msg['label.DicType.REMARK']}</th>
			<th>${msg['label.DicType.TYPE']}</th>
			<th>${msg['label.DicType.STATUS']}</th>
			<th>${msg['label.DicType.UPDATE_BY']}</th>
			<th>${msg['label.DicType.UPDATE_TIME']}</th>
			<th>${msg['label.DicType.PARENT_ID']}</th>
		...
		<c:forEach items="${list}" var="tag" varStatus="id">
			<tr class="tr_data">
				...
				<td nowrap>
					...
					<a	href="<%=path%>/DicType/view?id=${tag.id}" data-toggle="tooltip"
					data-placement="right" title="${tag.remark}">
					${tag.value_type}
					</a>
					</span>
				</td>
				...
				<td nowrap>${tag.parent_id}</td>
				<td nowrap><a href="<%=path%>/DicValue/list?typeId=${tag.id}" />${msg['label.OPERATION.DETAIL']}</a>
				<span class="divider">|</span> <a
					href="<%=path%>/DicType/edit?id=${tag.id}" /><i class="icon-edit"></i></a>
				</td>
			</tr>
		</c:forEach>
...
<script language="javascript">
		...
		function doCheck(form,ids){
			...
			if(count > 0){
				form.action = "<%=path%>/DicType/check?";
				form.submit();
			}else{
				alert("请选择记录！");
			}
