...
<table class="table table-bordered">
	...
	<tbody>
		...
		<c:forEach items="${page.list}" var="tag" varStatus="id">
			<tr class="tr_data">
				...
				<td nowrap title="${tag.remark}"><a
					href="<%=path%>/CostFill/view?id=${tag.id}">${tag.name}</a></td>
				<td nowrap>${tag.start_value}</td>
				<td nowrap>${tag.end_value}</td>
				...
				<td nowrap><a href="<%=path%>/CostFill/view?id=${tag.id}" />${msg['label.OPERATION.VIEW']}</a>
		...
		<tr class="tr_data">
			<td colspan="15" align="center"><jsp:include
