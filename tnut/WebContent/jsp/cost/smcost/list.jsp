...
<ul class="breadcrumb">
	...
	<span class="divider">/</span></li>
	<li class="active">${msg['label.SmCost.TABLE_CN']}${msg['display.label.list']}</li>
</ul>
</div>
...
<form action="<%=path%>/SmCost/list?" method="post" id="listForm"
	name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
			...
			<th>#</th>
			<th>${msg['label.SmCost.HAPPENDATE']}</th>
			<th>${msg['label.SmCost.NAME']}</th>
			<th>${msg['label.SmCost.TYPE']}</th>
			<th>${msg['label.SmCost.FILLUSER']}</th>
			<th>${msg['label.SmCost.PRICE']}</th>
			<th>${msg['label.SmCost.QTY']}</th>
			<th>${msg['label.SmCost.AMOUNT']}</th>
			<th>${msg['label.SmCost.UNIT']}</th>
			<th>${msg['label.SmCost.STATUS']}</th>
		...
		<c:forEach items="${page.list}" var="tag" varStatus="id">
			<tr class="tr_data">
				...
				<td nowrap title="${tag.remark}"><a
					href="<%=path%>/SmCost/view?id=${tag.id}">${tag.name}</a></td>
				...
				<td nowrap><a href="<%=path%>/SmCost/view?id=${tag.id}" />${msg['label.OPERATION.VIEW']}</a>
				<c:if test="${tag.status =='0' || tag.status=='2'}">
					<span class="divider">|</span>
					<a href="<%=path%>/SmCost/edit?id=${tag.id}" /><i
						class="icon-edit"></i></a>
				</c:if></td>
			</tr>
		</c:forEach>
		<tr class="tr_data">
			<td colspan="14" align="center"><jsp:include
