...
<form action="<%=path%>/SmStatterm/list?" method="post" id="listForm"
	name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
			...
			<th>#</th>
			<th>${msg['label.SmStatterm.CODE']}</th>
			<th>${msg['label.SmStatterm.NAME']}</th>
			<th>${msg['label.SmStatterm.TYPE']}</th>
			<th>${msg['label.SmStatterm.STARTDATE']}</th>
			<th>${msg['label.SmStatterm.ENDDATE']}</th>
			<th>${msg['label.SmStatterm.STATUS']}</th>
		...
		<c:forEach items="${page.list}" var="tag" varStatus="id">
			<tr class="tr_data">
				...
				<td nowrap title="${tag.remark}"><a
					href="<%=path%>/SmStatterm/view?id=${tag.id}">${tag.name}</a></td>
				<td nowrap>${tag.type}</td>
				<td nowrap>${tag.startdate}</td>
				<td nowrap>${tag.enddate}</td>
				...
				<td nowrap><a href="<%=path%>/SmStatterm/view?id=${tag.id}" />${msg['label.OPERATION.VIEW']}</a>
				<c:if test="${tag.status =='0' || tag.status=='2'}">
					<span class="divider">|</span>
					<a href="<%=path%>/SmStatterm/edit?id=${tag.id}" /><i
						class="icon-edit"></i></a>
				</c:if></td>
			</tr>
		</c:forEach>
