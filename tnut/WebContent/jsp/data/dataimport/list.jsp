...
<span class="divider">/</span></li>
<li class="active">${msg['label.DataimportInfo.TABLE_CN']}${msg['display.label.list']}</li>
</ul>
</div>
...
<form action="<%=path %>/DataimportInfo/list?" method="post"  id="listForm" name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
		<th><input type="checkbox" name="all" onclick="selectAll('ids')"/></th>
		<th>#</th>
		<th>${msg['label.DataimportInfo.CODE']}</th>
		<th>${msg['label.DataimportInfo.NAME']}</th>
		<th>${msg['label.DataimportInfo.TYPE']}</th>
		<th>${msg['label.DataimportInfo.STATUS']}</th>
		<th>${msg['label.DataimportInfo.IS_READ']}</th>
...
<td nowrap>${id.count}</td>
			...
			<td nowrap title="${tag.remark}"><a href="<%=path %>/DataimportInfo/view?id=${tag.id}">${tag.name}</a></td>
			...
			<td nowrap>${tag.is_read}</td>
			<td nowrap>
				<a href="<%=path %>/DataimportInfo/view?id=${tag.id}"/>${msg['label.OPERATION.VIEW']}</a>
				<c:if test="${tag.status =='0' || tag.status=='2'}">
						<span class="divider">|</span>
						<a href="<%=path%>/DataimportInfo/edit?id=${tag.id}" /><i class="icon-edit"></i></a>
				</c:if>
				<span class="divider">|</span>
				<a href="<%=path %>/DataimportPart/list?pid=${tag.id}"/>${msg['display.label.detail']}</a>
			</td>
	</tr>
</c:forEach>
<tr class="tr_data">
