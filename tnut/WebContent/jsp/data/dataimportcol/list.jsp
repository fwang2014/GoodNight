...
<span class="divider">/</span></li>
<li class="active">${msg['label.DataimportCol.TABLE_CN']}${msg['display.label.list']}</li>
</ul>
</div>
...
<form action="<%=path %>/DataimportCol/list?" method="post"  id="listForm" name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
		<th><input type="checkbox" name="all" onclick="selectAll('ids')"/></th>
		<th>#</th>
		<th>${msg['label.DataimportCol.CODE']}</th>
		<th>${msg['label.DataimportCol.NAME']}</th>
		<th>${msg['label.DataimportCol.TYPE']}</th>
		<th>${msg['label.DataimportCol.STATUS']}</th>
		<th>${msg['label.DataimportCol.IMPORT_PART_ID']}</th>
...
<td nowrap>${id.count}</td>
			...
			<td nowrap title="${tag.remark}"><a href="<%=path %>/DataimportCol/view?id=${tag.id}">${tag.name}</a></td>
		...
		<td nowrap><a href="<%=path %>/DataimportCol/view?id=${tag.id}"/>${msg['label.OPERATION.VIEW']}</a>
<c:if test="${tag.status =='0' || tag.status=='2'}">
		<span class="divider">|</span>
		<a href="<%=path%>/DataimportCol/edit?id=${tag.id}" /><i class="icon-edit"></i></a>
</c:if>
	</td>
	</tr>
</c:forEach>
