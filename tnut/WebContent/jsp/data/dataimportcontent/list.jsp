...
<span class="divider">/</span></li>
<li class="active">${msg['label.DataimportContent.TABLE_CN']}${msg['display.label.list']}</li>
</ul>
</div>
...
<form action="<%=path %>/DataimportContent/list?" method="post"  id="listForm" name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
		<th><input type="checkbox" name="all" onclick="selectAll('ids')"/></th>
		<th>#</th>
		<th>${msg['label.DataimportContent.CODE']}</th>
		<th>${msg['label.DataimportContent.NAME']}</th>
		<th>${msg['label.DataimportContent.CONTENT']}</th>
		<th>${msg['label.DataimportContent.TYPE']}</th>
		
		
		<th>${msg['label.DataimportContent.STATUS']}</th>	
...
<td nowrap>${id.count}</td>
			...
			<td nowrap title="${tag.remark}"><a href="<%=path %>/DataimportContent/view?id=${tag.id}">${tag.name}</a></td>
			
			
			<td nowrap>${tag.content}</td>
		...
		<td nowrap><a href="<%=path %>/DataimportContent/view?id=${tag.id}"/>${msg['label.OPERATION.VIEW']}</a>
<c:if test="${tag.status =='0' || tag.status=='2'}">
		<span class="divider">|</span>
		<a href="<%=path%>/DataimportContent/edit?id=${tag.id}" /><i class="icon-edit"></i></a>
</c:if>
	</td>
	</tr>
</c:forEach>
