...
<span class="divider">/</span></li>
<li class="active">${msg['label.DataimportPart.TABLE_CN']}${msg['display.label.list']}</li>
</ul>
</div>
...
<form action="<%=path %>/DataimportPart/list?" method="post"  id="listForm" name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
		<th><input type="checkbox" name="all" onclick="selectAll('ids')"/></th>
		<th>#</th>
		<th>${msg['label.DataimportPart.CODE']}</th>
		<th>${msg['label.DataimportPart.NAME']}</th>
		<th>${msg['label.DataimportPart.TYPE']}</th>
		
		<th>${msg['label.DataimportPart.IMPORT_ID']}</th>
		<th>${msg['label.DataimportPart.STATUS']}</th>
...
<td nowrap>${id.count}</td>
			...
			<td nowrap title="${tag.remark}"><a href="<%=path %>/DataimportPart/view?id=${tag.id}">${tag.name}</a></td>
			<td nowrap>${tag.type}</td>
				
			<td nowrap>${tag.import_id}</td>
			<td nowrap>
			...
			<td nowrap>
				<a href="<%=path %>/DataimportPart/view?id=${tag.id}"/>${msg['label.OPERATION.VIEW']}</a>
				<c:if test="${tag.status =='0' || tag.status=='2'}">
						<span class="divider">|</span>
						<a href="<%=path%>/DataimportPart/edit?id=${tag.id}" /><i class="icon-edit"></i></a>
				</c:if>
				<span class="divider">|</span>
				<a href="<%=path %>/DataimportContent/list?pid=${tag.id}"/>${msg['display.label.detail']}</a>
			</td>
	</tr>
</c:forEach>
<tr class="tr_data">
