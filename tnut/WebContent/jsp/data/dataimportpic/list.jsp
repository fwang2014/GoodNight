...
<ul class="dropdown-menu">
	...
	<li><a href="#myModal" role="button" class="btn"
		data-toggle="modal"><i class="icon-tag"></i>${msg['operate.search']}</a></li>

</ul>
</div>
<!-- /btn-group --></div>

<form action="<%=path %>/DataimportPic/list?" method="post"  id="listForm" name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
		<th><input type="checkbox" name="all" onclick="selectAll('ids')"/></th>
		<th>#</th>
		<th>${msg['label.DataimportPic.CODE']}</th>
		<th>${msg['label.DataimportPic.NAME']}</th>
		<th>${msg['label.DataimportPic.TYPE']}</th>
		<th>${msg['label.DataimportPic.STATUS']}</th>
		<th>${msg['label.DataimportPic.IMPORT_PART_ID']}</th>
		<th>${msg['label.DataimportPic.PIC_NAME']}</th>
		<th>${msg['label.DataimportPic.PIC_TYPE']}</th>
		<th>${msg['label.DataimportPic.PIC_SIZE']}</th>
		<th>${msg['label.DataimportPic.PIC_TEM_PATH']}</th>
		<th>${msg['label.DataimportPic.PIC_SAVE_PATH']}</th>
...
<td nowrap>${id.count}</td>
			...
			<td nowrap title="${tag.remark}"><a href="<%=path %>/DataimportPic/view?id=${tag.id}">${tag.name}</a></td>
			...
			<td nowrap>${tag.import_part_id}</td>
			<td nowrap>${tag.pic_name}</td>
			<td nowrap>${tag.pic_type}</td>
			<td nowrap>${tag.pic_size}</td>
			<td nowrap>${tag.pic_tem_path}</td>
			<td nowrap>${tag.pic_save_path}</td>
		<td nowrap><a href="<%=path %>/DataimportPic/view?id=${tag.id}"/>${msg['label.OPERATION.VIEW']}</a>
<c:if test="${tag.status =='0' || tag.status=='2'}">
		<span class="divider">|</span>
		<a href="<%=path%>/DataimportPic/edit?id=${tag.id}" /><i class="icon-edit"></i></a>
</c:if>
	</td>
	</tr>
</c:forEach>
