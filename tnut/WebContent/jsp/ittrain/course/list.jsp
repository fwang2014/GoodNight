...
<ul class="breadcrumb">
	...
	<li><a href="<%=path%>/Course/list?" class="label label-info">IT${msg['label.Course.TABLE_CN']}</a>
	<span class="divider">/</span></li>
	<li class="active">${msg['label.Course.TABLE_CN']}${msg['display.label.list']}</li>
</ul>
</div>
...
<ul class="nav">
	<li class="active"><a href="<%=path%>/Course/list?">${msg['label.Course.TABLE_CN']}${msg['display.label.list']}</a></li>
	<li><a href="<%=path%>/Course/add?">${msg['label.Course.TABLE_CN']}${msg['display.label.add']}</a></li>
</ul>
</div>
</div>
</div>
...
<form action="<%=path%>/Course/list?" method="post" id="listForm"
	name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
			...
			<th>#</th>
			<th>${msg['label.Course.CODE']}</th>
			<th>${msg['label.Course.NAME']}</th>
			<th>${msg['label.Course.TYPE']}</th>
			<th>${msg['label.Course.STATUS']}</th>
			<th>${msg['label.Course.STUDY_CONDITION']}</th>
	...
	<tbody>
		...
		<c:forEach items="${page.list}" var="tag" varStatus="id">
			<tr class="tr_data">
				...
				<td nowrap title="${tag.remark}"><a href="<%=path%>/Course/view?id=${tag.id}">${tag.name}</a></td>
				<td nowrap>${tag.type_name}</td>
				<td nowrap>${tag.status_name}</td>
				<td nowrap>${tag.study_condition}</td>
				<td nowrap><a href="<%=path%>/Course/view?id=${tag.id}" />${msg['label.OPERATION.VIEW']}</a>
				
				<c:if test="${tag.status =='0'}">
					<span class="divider">|</span> 
					<a	href="<%=path%>/Course/edit?id=${tag.id}" /><i class="icon-edit"></i></a>
				</c:if>
