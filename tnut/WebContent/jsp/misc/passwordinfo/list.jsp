...
<ul class="nav">
<li class="active"><a href="<%=path%>/PasswordInfo/list?">${msg['label.PasswordInfo.TABLE_CN']}${msg['display.label.list']}</a></li>
<li><a href="<%=path%>/PasswordInfo/add?">${msg['label.PasswordInfo.TABLE_CN']}${msg['display.label.add']}</a></li>
<li><a href="<%=path%>/PasswordInfo/userlist?">${msg['label.User.TABLE_CN']}${msg['display.label.list']}</a></li>
</ul>
</div>
</div>
</div>
...
<form action="<%=path%>/PasswordInfo/list?" method="post" id="listForm"
	name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
			...
			<th>#</th>
			<th>${msg['label.PasswordInfo.USER_ID']}</th>
			<th>${msg['label.PasswordInfo.CODE']}</th>
			<th>${msg['label.PasswordInfo.NAME']}</th>
			<th>${msg['label.PasswordInfo.TYPE']}</th>
			<th>${msg['label.PasswordInfo.STATUS']}</th>
		...
		<c:forEach items="${page.list}" var="tag" varStatus="id">
			<tr class="tr_data">
				...
				<td nowrap title="${tag.remark}"><a
					href="<%=path%>/PasswordInfo/view?id=${tag.id}">${tag.name}</a></td>
				...
				<td nowrap><a href="<%=path%>/PasswordInfo/view?id=${tag.id}" />${msg['label.OPERATION.VIEW']}</a>
				<c:if test="${tag.status =='0' || tag.status=='2'}">
					<span class="divider">|</span>
					<a href="<%=path%>/PasswordInfo/edit?id=${tag.id}" /><i
						class="icon-edit"></i></a>
				</c:if></td>
			</tr>
		</c:forEach>
