...
<ul class="breadcrumb">
	...
	<span class="divider">/</span></li>
	<li class="active">${msg['label.Phonebook.TABLE_CN']}${msg['display.label.list']}</li>
</ul>
</div>
...
<form action="<%=path%>/Phonebook/list?" method="post" id="listForm"
	name="listForm">
		...
		<c:forEach items="${page.list}" var="tag" varStatus="id">
			<tr class="tr_data">
				...
				<td nowrap title="${tag.remark}"><a
					href="<%=path%>/Phonebook/view?id=${tag.id}">${tag.name}</a></td>
				...
				<td nowrap><a href="<%=path%>/Phonebook/view?id=${tag.id}" />${msg['label.OPERATION.VIEW']}</a>
				<c:if test="${tag.status =='0' || tag.status=='2'}">
					<span class="divider">|</span>
					<a href="<%=path%>/Phonebook/edit?id=${tag.id}" /><i
						class="icon-edit"></i></a>
				</c:if></td>
			</tr>
		</c:forEach>
