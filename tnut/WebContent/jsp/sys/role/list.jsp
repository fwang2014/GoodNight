...
<ul class="dropdown-menu">
	...
	<li class="divider"></li>
	<li><a href="#" onclick="doCheck('listForm', 'ids')"
><i
...
<form action="<%=path%>/Role/list?" method="post" id="listForm"
	name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
			...
			<th>#</th>
			<th>${msg['label.Role.CODE']}</th>
			<th>${msg['label.Role.NAME']}</th>
			<th>${msg['label.Role.TYPE']}</th>
			<th>${msg['label.Role.STATUS']}</th>
	...
	<tbody>
		...
		<c:forEach items="${page.list}" var="tag" varStatus="id">
			<tr class="tr_data">
				...
				<td nowrap title="${tag.remark }"><a href="<%=path%>/Role/view?id=${tag.id}">${tag.name}</a></td>
				<td nowrap>${tag.type}</td>
				<td nowrap>${tag.status}</td>
				<td nowrap><a href="<%=path%>/Role/view?id=${tag.id}" />${msg['label.OPERATION.VIEW']}</a>
				<c:if test="${tag.status =='0'}">
					<span class="divider">|</span>
					<a href="<%=path%>/Role/edit?id=${tag.id}" /><i class="icon-edit"></i></a>
				</c:if></td>
			</tr>
		</c:forEach>
		<tr class="tr_data">
