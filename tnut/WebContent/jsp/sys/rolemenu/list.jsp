...
<form action="<%=path%>/RoleMenu/list?" method="post" id="listForm"
	name="listForm">
<table class="table table-bordered">
	<thead>
		<tr>
			...
			<th>#</th>
			<th>${msg['label.RoleMenu.CODE']}</th>
			<th>${msg['label.RoleMenu.NAME']}</th>
			<th>${msg['label.RoleMenu.TYPE']}</th>
			<th>${msg['label.RoleMenu.STATUS']}</th>
			<th>${msg['label.RoleMenu.ROLE_ID']}</th>
			<th>${msg['label.RoleMenu.MENU_ID']}</th>
		...
		<c:forEach items="${page.list}" var="tag" varStatus="id">
			<tr class="tr_data">
				...
				<td nowrap title="${tag.remark}"><a
					href="<%=path%>/RoleMenu/view?id=${tag.id}">${tag.name}</a></td>
				<td nowrap>${tag.type}</td>
				<td nowrap>${tag.status}</td>
				<td nowrap>${tag.role_id}</td>
				<td nowrap>${tag.menu_id}</td>
				<td nowrap><a href="<%=path%>/RoleMenu/view?id=${tag.id}" />${msg['label.OPERATION.VIEW']}</a>
				<c:if test="${tag.status =='0'}">
					<span class="divider">|</span>
					<a href="<%=path%>/RoleMenu/edit?id=${tag.id}" /><i
						class="icon-edit"></i></a>
				</c:if></td>
			</tr>
		</c:forEach>
