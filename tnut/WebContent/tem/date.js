...
document.writeln('<iframe id=meizzDateLayer scrolling="no" Author=wayx frameborder=0 style="position: absolute; width: 144; height: 211; z-index: 9998; display: none"></iframe>');
strFrame='<style>';
strFrame+='INPUT.button{BORDER-RIGHT: #84A2C6 1px solid;BORDER-TOP: #84A2C6 1px solid;BORDER-LEFT: #84A2C6 1px solid;';
strFrame+='BORDER-BOTTOM: #84A2C6 1px solid;BACKGROUND-COLOR:  #84A2C6;font-family:宋体;}';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=160 bordercolor=#84A2C6 bgcolor=#84A2C6 Author="wayx">';
...
strFrame+='      <tr align=center Author="wayx"><td width=16 align=center bgcolor=#84A2C6 style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='        onclick="parent.meizzPrevM()" title="向前翻 1 月" Author=meizz><b Author=meizz>&lt;</b>';
strFrame+='        </td><td width=60 align=center style="font-size:12px;cursor:default" Author=meizz ';
strFrame+='onmouseover="style.backgroundColor=\'#CECFCE\'" onmouseout="style.backgroundColor=\'white\'" ';
...
strFrame+='<td width=48 align=center style="font-size:12px;cursor:default" Author=meizz onmouseover="style.backgroundColor=\'#CECFCE\'" ';
strFrame+=' onmouseout="style.backgroundColor=\'white\'" onclick="parent.tmpSelectMonthInnerHTML(this.innerText.length==3?this.innerText.substring(0,1):this.innerText.substring(0,2))"';
strFrame+='        title="点击这里选择月份"><span id=meizzMonthHead Author=meizz></span></td>';
strFrame+='        <td width=16 bgcolor=#84A2C6 align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 bgcolor=#84A2C6 ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':'');
strFrame+=' BORDERCOLORLIGHT=#84A2C6 BORDERCOLORDARK=#FFFFFF width=140 height=20 Author="wayx" style="cursor:' + (bMoveable ? 'move':'default') + '">';
...
strFrame+='    <table border=1 cellspacing=2 cellpadding=0 BORDERCOLORLIGHT=#84A2C6 BORDERCOLORDARK=#FFFFFF bgcolor= #84A2C6 width=140 height=120 Author="wayx">';
...
strFrame+='        <td colspan=3 align=right Author=meizz width="100%">';
//wujing edit
strFrame+='<span onclick=parent.clearDate() style="font-size:12px;cursor:hand;width:50%" title="清空">';
strFrame+='<u>清空</u></span>';

strFrame+='<span onclick=parent.closeLayer() style="font-size:12px;cursor: hand;width:50%"';
...
function clearDate()
{
     window.document.all(outObject).value='';
     closeLayer();
}
...
function setday(tt,obj) //主调函数
{
	...
	if (document.all(outObject)){
		var r = document.all(outObject).value.match(reg);
	}else{
		var r = document.getElementById(outObject).value.match(reg);
	}
	if(r!=null){
		r[2]=r[2]-1;
...
{
  with(window.event)
  { if (srcElement.getAttribute("Author")==null && srcElement != document.all(outObject) && srcElement != outButton)
    closeLayer();
  }
}
...
function document.onkeyup()		//按Esc键关闭，切换焦点关闭
  {
    if (window.event.keyCode==27){
		if(document.all(outObject))document.all(outObject).blur();
		closeLayer();
	}
	else if(document.activeElement)
		if(document.activeElement.getAttribute("Author")==null && document.activeElement != document.all(outObject) && document.activeElement != outButton)
		{
			closeLayer();
		}
  }
...
function meizzToday()  //Today Button
  {
    ...
    //meizzSetDay(meizzTheYear,meizzTheMonth);
    if(document.all(outObject)){
		document.all(outObject).value=meizzTheYear + "-" + meizzTheMonth + "-" + today;
    }
    closeLayer();
  }
    ...
    if (meizzWDay[i]!="")
      {
		...
		if(i<firstday)		//上个月的部分
		{
			...
			else
			{
				...
				meizzWDay[i] == new Date().getDate()) ? "#CECFCE":"#e0e0e0");
				//将选中的日期显示为凹下去
				if((mm==1?yy-1:yy)==outDate.getFullYear() && (mm==1?12:mm-1)== outDate.getMonth() + 1 &&
				meizzWDay[i]==outDate.getDate())
				{
					da.borderColorLight="#FFFFFF";
					da.borderColorDark="#84A2C6";
				}
			}
		}
...
function meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  ...
  if (document.all(outObject))
  {
    if (!n) {//document.all(outObject).value="";
      return;}
    if ( n < 10){n = "0" + n;}
   if(!document.all(outObject).disabled){
    document.all(outObject).value= yy + "-" + mm + "-" + n ; //注：在这里你可以输出改成你想要的格式
   }
    closeLayer();
  }
